...
- **Trigger name**: `trg_vaccination_{descriptive_name}`
- **Function name**: `trg_vaccination_{descriptive_name}_fn`

Where `vaccination` is your plugin's moduleId (pluginId with hyphens replaced by underscores).
...
CREATE OR REPLACE FUNCTION "trg_vaccination_audit_log_fn"()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER "trg_vaccination_audit_log"
AFTER INSERT OR UPDATE ON "module_vaccination_items"
FOR EACH ROW
EXECUTE FUNCTION "trg_vaccination_audit_log_fn"();
```

> **Important**: The system automatically **DISABLE**s all `trg_vaccination_*` triggers when the plugin
